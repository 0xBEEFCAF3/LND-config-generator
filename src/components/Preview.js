    ...
    return (
      <div className="mdl-card mdl-shadow--2dp preview-card">
        <div className="mdl-card__title">
          <div className="preview-title mdl-card__title-text">lnd.conf</div>
        </div>
        <div className="mdl-card__actions mdl-card--border">
          <textarea
            className="preview-editor"
            readOnly
            value={toConf(settings, defaults)}
          />
        </div>
        <div className="mdl-card__menu">
          ...
          >
            <i className="material-icons" id="link">
              link
            </i>
            <span className="mdl-tooltip" htmlFor="link">
              Link to this Config File
            </span>
          </a>
          <button
            className="mdl-button mdl-button--icon mdl-js-button mdl-js-ripple-efect"
          ...
          >
            <i className="material-icons" id="download">
              cloud download
            </i>
            <span className="mdl-tooltip" htmlFor="download">
...
function toConf(settings, defaults) {
  const conf = Object.keys(settings)
    ...
    .reduce((acc, section) => {
      ...
      const vals = Object.keys(settings[section])
        .filter(
          (key) => !isEqual(settings[section][key], defaults[section][key])
        )
        .map((key) => {
  ...
  const configPath = joinPath([basePath(platform), "lnd.conf"], platform);
  conf.unshift(
    // '# Generated by https://0xBEEFCAF3/LND-config-generator/\n',
